...
export default function IncompleteIngredients({navigation}) {
  ...
  const fetchIngredients = async () => {
    try{
      const response = await axios.get( baseUrl+ '/incomplete', {timeout:20});
      ...
      const handleSearch = async () => {
        ...
        all_ingredients.map((i:any) => {
          if(i.name == search && i.complete == 'NO'){
  ...
  return (
    ...
    <Layout style={styles.container}>
      <Text style={styles.text}>Incomplete ingredients information:</Text>
      <Text>Search:</Text>
      <Input
