...
export default function ExpiringIngredients({navigation}) {
  ...
  const fetchIngredients = async () => {
      try{
      const response = await axios.get(baseUrl + '/expiring', {timeout: 20});

      setIngredients(response.data.response_body)
      }catch(e){
        console.log('hellor')
              let expiring : any = await AsyncStorage.getItem('expiring');
  ...
  return (
    <Layout style={styles.container}>
      <Text style={styles.text}>Expiring ingredients:</Text>
